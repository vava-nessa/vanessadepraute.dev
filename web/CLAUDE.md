# CLAUDE

Always read this file before starting any task in this repo.
Then read `AGENTS.md` in this folder for the full project context.

- **CRITICAL**: It is very important to read the README files and strictly follow their indications.
- **ALWAYS** read the README and **ALWAYS** use pnpm only.
- **NEVER** commit or push changes automatically - let the user do it.
- The website lives in this folder (see `README.md` for details).

