...
const Background: React.FC<BackgroundProps> = ({ className }) => {
  ...
  const yOff = 0.00125;
  const zOff = 0.0005;
  const backgroundColor = "hsla(260,40%,5%,0.5)";
  ...
  useEffect(() => {
    try {
      ...
      canvasB.style.zIndex = "-1"; // Ensure it's behind other content
      ...
      const initParticle = (i: number) => {
        try {
          ...
          const y = centerRef.current[1] + utils.randRange(rangeY);
          const vx = 0;
          const vy = 0;
          const life = 0;
          const ttl = baseTTL + utils.rand(rangeTTL);
          const speed = baseSpeed + utils.rand(rangeSpeed);
          const radius = baseRadius + utils.rand(rangeRadius);
          const hue = baseHue + utils.rand(rangeHue);
      ...
      console.error("Error in Background component:", error);
    }
  }, []);
